...
function IntraWire({ isOpen }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      let url = `http://localhost:3000/payout_partner_wire/view?startDate=${startDate}&endDate=${endDate}&page=${page}&pageSize=${dataPerPage}`;
      if (mto !==''){
        url += `&filterValue=${mto}`;
      }
  ...
  const handleExport = () => {
    const fetchallData = async () => {
      try {
        const url = `http://localhost:3000/payout_partner_wire/view?startDate=${startDate}&endDate=${endDate}`;
        const response = await fetch(url);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
  ...
  return (
    <div className={`mycontents ${isOpen ? '' : 'close'} px-5 py-2`}>
      <div className='myfont py-4 uppercase'>Payout Partner Wire</div>
